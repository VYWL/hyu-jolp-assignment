...
def perform_insertion_and_validation(avl_tree, filename):
    data = read_csv(filename)
    for key, value in tqdm(data, desc="Inserting data"):
        avl_tree.insert_node(int(key), int(value))

    save_tree_to_csv(avl_tree, 'inserted_data.csv')
    validate_inserted_data('inserted_data.csv', filename)

def perform_deletion_and_validation(avl_tree, delete_filename, compare_filename):
    delete_data = read_csv(delete_filename)
    deleted_keys = set()

    for key in tqdm(delete_data, desc="Deleting data"):
        key_int = int(key[0])
        avl_tree.delete_node(key_int)
        deleted_keys.add(key_int)

    save_tree_deletion(avl_tree, deleted_keys, 'remaining_after_deletion.csv')
